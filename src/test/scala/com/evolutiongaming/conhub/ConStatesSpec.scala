...
class ConStatesSpec extends AnyWordSpec with ActorSpec with Matchers with ConHubSpecHelper {
  ...
  "ConStates" should {
    ...
    "disconnect" in new Scope {
      ...
      conStates.disconnect(id, version, reconnectTimeout).get shouldEqual NotUpdated(
        Reason.UpdateNotDefinedForValue(None)
      )
      state shouldEqual None
      ...
      conStates.disconnect(id, version.dec, reconnectTimeout, Ctx.Remote(address)).get shouldEqual NotUpdated(
